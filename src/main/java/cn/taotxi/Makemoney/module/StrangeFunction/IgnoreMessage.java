...
public class IgnoreMessage {
    ...
    public static void addPresetIgnoreList() {
        List<String> list = List.of(
            "^\\[拾玖福彩\\]",
            "^【猜单词游戏】$|^拾玖喵不太认识这个单词：|^提示：|^用 /word <你的猜测> 回答本题（每人仅一次）$|^当前词库：\\w+（共 \\d+ 条）$|^----------------------$",
            "^地震信息$|^ 20\\d{2}年\\d{2}月\\d{2}日 \\d{2}时\\d{2}分\\d{2}秒 发生$|^ 震中: |^ 震级: (?:\\d+(?:\\.\\d+)?)?$|^ 深度: \\d+(?:\\.\\d+)?km$|^ 最大震度: (?:\\d+|不明)$|^ 海啸信息: |^ 最大烈度: \\d+$|^ 更新时间: \\d{4}[-/]\\d{2}[-/]\\d{2} \\d{2}:\\d{2}:\\d{2}$|地震.*? \\| 第\\d{1,2}报|^中国地震台网 \\((?:正式|自动)测定\\)$",
            "^拾玖喵小道消息",
            "^拾玖喵次元口袋",
            "^别忘了去看看，奖励多多别错过喵~",
            "^$",
