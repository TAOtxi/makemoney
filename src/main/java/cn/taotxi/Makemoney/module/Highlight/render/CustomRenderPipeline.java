...
public class CustomRenderPipeline {
    ...
    public void startDrawing() {
        ...
        draw(Minecraft.getInstance(), FILLED_THROUGH_WALLS, builtBuffer, drawParameters, vertices, format);
    ...
    private GpuBuffer upload(MeshData.DrawState drawParameters, VertexFormat format, MeshData builtBuffer) {
        ...
        if (vertexBuffer == null || vertexBuffer.size() < vertexBufferSize) {
            ...
            vertexBuffer = new MappableRingBuffer(() -> Highlight.MODULE_NAME + " example render pipeline", GpuBuffer.USAGE_VERTEX | GpuBuffer.USAGE_MAP_WRITE, vertexBufferSize);
        }
    ...
    private static void draw(Minecraft client, RenderPipeline pipeline, MeshData builtBuffer, MeshData.DrawState drawParameters, GpuBuffer vertices, VertexFormat format) {
        ...
        try (RenderPass renderPass = RenderSystem.getDevice()
                ...
                .createRenderPass(() -> Highlight.MODULE_NAME + " example render pipeline rendering", client.getMainRenderTarget().getColorTextureView(), OptionalInt.empty(), client.getMainRenderTarget().getDepthTextureView(), OptionalDouble.empty())) {
